...
const APIRepositoryDetails = () => {
  ...
  const exportHeaders = () => {
    const activeHeaders = requestConfig.headers.filter(
      (h) => h.enabled && h.key.trim()
    );
    const headerString = activeHeaders
      .map((h) => `${h.key}: ${h.value}`)
      .join('\n');
  ...
  const exportParams = () => {
    const activeParams = requestConfig.params.filter(
  ...
  const importParams = () => {
    const input = prompt(
      'Paste parameters (one per line, format: "key=value"):'
    );
    if (input) {
      try {
        const lines = input.trim().split('\n');
        const newParams = lines
          .map((line) => {
            const [key, ...valueParts] = line.split('=');
            return {
              key: key?.trim() || '',
      ...
      const fetchApiDetails = async () => {
        try {
          ...
          if (response && response.result && response.result.data) {
            ...
            const headers = Object.entries(apiData.request.headers || {}).map(
              ([key, value]) => ({
                key,
                value,
                enabled: true,
            ...
            if (apiData.request.body) {
              ...
              bodyConfig.contentType =
                apiData.request.body.contentType || 'application/json';

              if (apiData.request.body.type === 'raw') {
                bodyConfig.raw = apiData.request.body.raw || '';
              } else if (
                apiData.request.body.type === 'form-data' &&
                apiData.request.body.formData
              ) {
                const formDataArray = Object.entries(
                  apiData.request.body.formData
                ).map(([key, data]) => ({
                  key,
                ...
                apiData.request.body.type === 'x-www-form-urlencoded' &&
                apiData.request.body.urlEncoded
              ) {
                const urlEncodedArray = Object.entries(
                  apiData.request.body.urlEncoded
                ).map(([key, value]) => ({
                  key,
                  value,
            ...
            const updatedConfig = {
              ...
              environment: apiData.environment?.environmentId || '',
          ...
          toast.error(
            'Failed to load API details: ' + (error.message || 'Unknown error')
          );
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      !requestConfig.auth.bearerToken.trim()
    ) {
      toast.warning(
    ...
    if (requestConfig.auth.type === 'Basic Auth') {
      if (
        !requestConfig.auth.username.trim() ||
        !requestConfig.auth.password.trim()
      ) {
        toast.warning(
          'Username and password are required when using Basic Auth'
        );
        return;
      }
    }
    ...
    if (requestConfig.auth.type === 'API Key') {
      if (
        !requestConfig.auth.apiKeyName.trim() ||
        !requestConfig.auth.apiKeyValue.trim()
      ) {
        toast.warning(
          'API Key name and value are required when using API Key authentication'
        );
        return;
      }
    }
  ...
  const copyResponsePart = (part, data) => {
    ...
    toast.success(
      `${part.charAt(0).toUpperCase() + part.slice(1)} copied to clipboard`
    );
  };
    ...
    switch (requestConfig.auth.type) {
      case 'Bearer Token':
        if (!requestConfig.auth.bearerToken.trim()) {
          ...
          const validation = validateBearerToken(
            requestConfig.auth.bearerToken
          );
          if (validation === 'invalid') {
            isValid = false;
            message =
              'Bearer token format appears invalid (JWT should have 3 parts)';
    ...
    if (detectedParams.length === 0) {
      return (
        <div className="text-sm text-gray-500 italic p-3 border-t">
          ...
          {'{paramName}'} syntax in URL, headers, params or body to create
          parameters.
        </div>
      );
    }
    ...
    <div className="p-6 space-y-6 bg-white min-h-screen">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="bg-white border rounded-md shadow-sm overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-12 gap-4 mb-4 items-start">
                ...
                <div className="col-span-8">{renderUrlInput()}</div>
          ...
          <div className="bg-white border rounded-md shadow-sm overflow-hidden">
            ...
            <div className="flex border-b">
              {[
                {
                  name: 'Params',
                  hasData: requestConfig.params.some(
                    (p) => p.enabled && p.key.trim()
                  ),
                },
                {
                  name: 'Headers',
                  hasData: requestConfig.headers.some(
                    (h) => h.enabled && h.key.trim()
                  ),
                },
                {
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {requestConfig.params.map((param, index) => (
                          <tr
                            key={index}
                            className={
                              !param.enabled ? 'bg-gray-50 text-gray-400' : ''
                            }
                          >
                            <td className="px-3 py-2">
                              <input
                                ...
                                onChange={() =>
                                  handleToggleEnabled('params', index)
                                }
                        ...
                        <tr>
                          ...
                          <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-2">
                            ...
                            <span
                              ...
                              title="Common headers will show suggestions"
                            >
                              ⓘ
                            </span>
                          </th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {requestConfig.headers.map((header, index) => (
                          <tr
                            key={index}
                            className={
                              !header.enabled ? 'bg-gray-50 text-gray-400' : ''
                            }
                          >
                            <td className="px-3 py-2">
                              <input
                                ...
                                onChange={() =>
                                  handleToggleEnabled('headers', index)
                                }
                                className="h-4 w-4 text-indigo-600 border-gray-300 rounded"
                              />
                            </td>
                        ...
                        {commonHeaders.slice(0, 8).map((headerName) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              const exists = requestConfig.headers.some(
                                (h) =>
                                  h.key.toLowerCase() ===
                              ...
                              if (!exists) {
                                ...
                                toast.info(
                                  `${headerName} header already exists`
                                );
                              }
                            }}
                  ...
                  {requestConfig.auth.type === 'Bearer Token' && (
                    <div className="mt-4">
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        The token will be added as: Authorization: Bearer
                        &lt;token&gt;
                      </p>
                    </div>
                  )}
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-4">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleAuthFieldChange('apiKeyAddTo', e.target.value)
                          }
                        ...
                        <p className="mt-1 text-xs text-gray-500">
                          API key will be added to{' '}
                          {requestConfig.auth.apiKeyAddTo.toLowerCase()} as:{' '}
                          {requestConfig.auth.apiKeyName || 'key-name'}:{' '}
                          {requestConfig.auth.apiKeyValue || 'key-value'}
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="bg-gray-100 rounded-full p-3 mx-auto w-fit mb-3">
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M9 12l2 2 4-4m5.02.02l.01.01M5 12l2 2L9 12m6.02-6.02l.01.01"
                          />
                        </svg>
                      </div>
                      ...
                      <div className="mb-2 flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleBodyContentTypeChange(e.target.value)
                            }
                        ...
                        {/* Body Tools */}
                        <div className="flex items-center gap-2">
                          {requestConfig.body.contentType ===
                            'application/json' && (
                            <>
                              <button
                                onClick={formatJSON}
                        ...
                        <div className="bg-gray-50 px-3 py-2 border-b flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            {requestConfig.body.raw && (
                              <span>
                                {requestConfig.body.raw.length} characters
                              </span>
                            )}
                            {jsonError && (
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-2 w-24">
                                Type
                              </th>
                              <th className="w-20 px-3 py-2"></th>
                            </tr>
                              ...
                              >
                                <td className="px-3 py-2">
                                  <input
                                    ...
                                    onChange={() =>
                                      handleToggleEnabled('formData', index)
                                    }
                                    className="h-4 w-4 text-indigo-600 border-gray-300 rounded"
                                  />
                                </td>
                                ...
                                <td className="px-3 py-2 text-right">
                                  <button
                                    onClick={() =>
                                      handleRemoveRow('formData', index)
                                    }
                                    className="text-gray-400 hover:text-red-500"
                                  >
                                    <FaMinus size={10} />
                    ...
                    <div>
                      ...
                      <div className="border rounded-md overflow-hidden">
                        <table className="min-w-full divide-y divide-gray-200">
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {requestConfig.body.urlEncoded.map(
                              (item, index) => (
                                <tr
                                  key={index}
                                  className={
                                ...
                                >
                                  <td className="px-3 py-2">
                                    <input
                                      ...
                                      checked={item.enabled}
                                      onChange={() =>
                                        handleToggleEnabled('urlEncoded', index)
                                      }
                                      className="h-4 w-4 text-indigo-600 border-gray-300 rounded"
                                    />
                                  </td>
                                  ...
                                  <td className="px-3 py-2 text-right">
                                    <button
                                      onClick={() =>
                                        handleRemoveRow('urlEncoded', index)
                                      }
                                      className="text-gray-400 hover:text-red-500"
                                    >
                                      <FaMinus size={10} />
          ...
          <div
            ref={responseRef}
            className="bg-white border rounded-md shadow-sm overflow-hidden max-w-full"
          >
            <div className="flex items-center justify-between p-4 border-b">
              ...
              {responseData.isLoading ? (
                ...
                <div className="bg-red-50 border border-red-200 rounded-md p-4">
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() =>
                        copyResponsePart('error', responseData.error)
                      }
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(responseData.status)}`}
                          >
                          ...
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {Object.entries(responseData.headers).map(
                                ([key, value]) => (
                                  <tr key={key}>
                                    <td className="px-3 py-2 text-sm font-medium text-gray-900 min-w-0">
                                      {key}
                                    </td>
                                    <td className="px-3 py-2 text-sm text-gray-600 break-all max-w-xs overflow-hidden">
                                      {value}
                                    </td>
                                    <td className="px-3 py-2 text-right">
                                      <button
