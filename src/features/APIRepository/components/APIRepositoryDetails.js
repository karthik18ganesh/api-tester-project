...
const APIRepositoryDetails = ({ initialData = {}, onSave, onCancel }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Only append query params for the enabled params that have values
      ...
      if (requestConfig.auth.type === "Bearer Token" && requestConfig.auth.bearerToken) {
        headers["Authorization"] = `Bearer ${requestConfig.auth.bearerToken}`;
      } else if (requestConfig.auth.type === "Basic Auth" && requestConfig.auth.username) {
        const encodedAuth = btoa(`${requestConfig.auth.username}:${requestConfig.auth.password}`);
      ...
      if (["POST", "PUT", "PATCH"].includes(requestConfig.method)) {
        if (requestConfig.body.type === "raw") {
          body = requestConfig.body.raw;
          headers["Content-Type"] = requestConfig.body.contentType;
      ...
      // Clone the response and handle based on content type
      ...
      if (contentType.includes("application/json")) {
        ...
        // For binary data, get it as a blob
        const blob = await response.blob();
        responseData = `Binary data: ${blob.type}, ${blob.size} bytes`;
  ...
  const handleSave = () => {
    ...
    // Call the onSave prop with the current state
    if (onSave) {
      onSave(requestConfig);
    }
  };
      ...
      <div className="bg-white border rounded-md shadow-sm overflow-hidden">
        <div className="flex items-center border-b p-2 bg-gray-50">
          <h2 className="text-lg font-semibold text-gray-800 px-2">
            {isEditMode ? "Edit API Request" : "New API Request"}
          </h2>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-end">
            <button
              onClick={handleSave}
              className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-md flex items-center gap-1 hover:bg-indigo-700"
            >
              <FiSave />
              <span>Save</span>
            </button>
