            ...
            {currentData.map((project) => (
              <tr key={project.id} className="border-b">
                ...
                <td className="py-3 px-4 text-right flex justify-end gap-3 pr-4">
                  <FaEdit className="cursor-pointer text-gray-600 hover:text-blue-500" onClick={() => handleEdit(project)} />
                  <FaTrash
          ...
          {[...Array(totalPages)].map((_, idx) => (
            <button
              ...
              className={`px-3 py-1 border rounded ${
                currentPage === idx + 1 ? "bg-indigo-600 text-white" : "hover:bg-gray-100"
              }`}
            >
              {idx + 1}
            </button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-30 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-lg w-[90%] max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Delete Project</h3>
            <p className="text-sm text-gray-700 mb-4">
              Are you sure you want to delete the project{" "}
              <span className="font-semibold text-red-600">"{selectedProject?.name}"</span>? This action cannot be undone.
            </p>
            <div className="flex justify-end gap-3">
              <button
