...
const TestSuiteAssignmentForm = () => {
  ...
  const handleSelectChange = (e) => {
    ...
    if (selected) {
      ...
      setAvailableCases(availableCases.filter((item) => item.id !== selected.id));
    }
  };
  ...
  const renderPagination = () => {
    ...
    return (
      <div className="flex justify-end items-center px-4 py-3 text-sm text-gray-600 gap-1">
        <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="px-3 py-1 border rounded hover:bg-gray-100 disabled:opacity-40">
          Prev
        </button>
        {start > 1 && <span>...</span>}
        {pages}
        {end < totalPages && <span>...</span>}
        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="px-3 py-1 border rounded hover:bg-gray-100 disabled:opacity-40">
          Next
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-white shadow p-6 rounded-lg mt-8">
      <h3 className="font-semibold text-sm text-gray-700 mb-2">
        ...
        <span className="ml-2 text-blue-500 text-xs bg-blue-100 px-2 py-1 rounded-full">ℹ Assign test cases</span>
      </h3>
      <label className="text-sm font-medium text-gray-600 mb-1 block">Select test case</label>
      <div className="flex items-center mb-4 space-x-2">
        <div className="flex flex-wrap gap-2 flex-1 border px-2 py-2 rounded">
          {selectedCases.map((test) => (
            <span key={test.id} className="bg-blue-100 text-blue-600 px-2 py-1 text-sm rounded-full flex items-center space-x-1">
              <span>{test.name}</span>
              <button onClick={() => handleRemoveBadge(test.id)} className="text-xs font-bold px-1">×</button>
            </span>
          ))}
          <select onChange={handleSelectChange} className="outline-none flex-1 bg-transparent">
      ...
      {tableData.length > 0 && (
        <>
          ...
          <div className="flex justify-end mt-4 space-x-3">
            <button onClick={handleCancel} className="px-4 py-2 border rounded text-sm text-gray-700 bg-white hover:bg-gray-100">
              Cancel
            </button>
            <button onClick={() => toast.success('Test cases associated')} className="px-4 py-2 bg-[#4F46E5] text-white text-sm rounded hover:bg-[#4338CA]">
              Create
            </button>
          </div>
        </>
